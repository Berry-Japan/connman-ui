                                ...
                                gpointer user_data)
{
	...
	if (gtk_status_icon_get_geometry(trayicon, &screen, &area, NULL)) {
		gtk_menu_popup_at_rect(cui_left_menu,
...
{
	// サービスリストのクリーンアップを遅延させる
	g_timeout_add(100, (GSourceFunc)connman_service_set_removed_callback, NULL);
}
